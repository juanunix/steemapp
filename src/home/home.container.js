...
export function HomeContainer(screenName) {
  const DB_KEY = screenName;
  const asyncFunctionName = `getDiscussionsBy${screenName}Async`
  class HomeScreen extends Component {
    ...
    getFeedArray = async () => {
      ...
      try {
        ...
        const feed = await steem.api[asyncFunctionName]({ tag: "", limit: 10 })
        await this.setState({
          feed,
          error:null,
          fetching: false,
    ...
    onEndReached = async (info) => {
      ...
      try {
        ...
        const nextFeed = await steem.api[asyncFunctionName](query)
